...
def pairwise_kstest(file,id_ind,val_ind,outfile):
	...
	for combo in combo_list:
		...
		try:
			pValue = stats.mannwhitneyu(all_list[ind1],all_list[ind2])
			pVal = pValue[1]
		except:
			print "\tERROR"
			pVal = "NA"
		outfile.write("%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\n" % (file.split("/")[-1],id_list[ind1],id_list[ind2],pVal,m1,m2,n1,n2))
...
if outfile_name.endswith("mwu_test"):
	out = open(outfile_name,"w")
else:
	out = open(outfile_name + ".mwu_test","w")
