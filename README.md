...
3. Now get arff files using your classes file and features file
		...
		python ~john3784/Github/MachineLearning-Weka/2_ARFF_from_matrix.py SMvsPMvsOther.ML_classes_file.txt metabolite SMvsPM_nogluc binary_matrix_2.0.mod.txt,binary continuous_matrix_2.0.txt,numeric categor_cluster_matrix.txt,categorical 
...
4. Balance ARFF files and generate parameter grid search commands: will create 100 random subsets of neg files, then create a all_bal_grid_searches.runcc to submit to the queue
	...
	python ~john3784/Github/MachineLearning-Weka/2_Machine_Learning/machine_learning_pipeline_1-grid_search_runcc-balanced_arffs.py

        python ~john3784/Github/MachineLearning-Weka/2_Machine_Learning/machine_learning_pipeline_1-grid_search_runcc-balanced_arffs.py -arff /mnt/home/john3784/2-specialized_metab_project/machine-learn_files/metabolite-SMvsPM_nogluc-binary_numeric_categorical-mod.arff -main_dir /mnt/home/john3784/2-specialized_metab_project/machine-learn_files/arff_files2.0/ -cmd /mnt/home/john3784/2-specialized_metab_project/machine-learn_files/metabolite.command 
...
submit runcc files
    ...
    python ~/Github/parse_scripts/qsub_hpc.py  -f submit -u john3784 -c all_bal_grid_searches.runcc -w 239 -m 12 -n 230
	
submit via Johnny's qsub for grouped submissions (typically 20-50 at a time)

	 python ~/Github/parse_scripts/qsub_hpc-JL.py -f submit -c all_bal_grid_searches.runcc -u john3784 -n 230 -w 239 -m 12 -nc 20
...
if not all jobs finished, run grid search again, then concatenate the bal_grid_search.failed.runcc files to resubmit as one file. Run with a longer walltime
        ...
        python ~/Github/parse_scripts/qsub_hpc.py  -f submit -u john3784 -c all_bal_grid_searches.failed.runcc -w 600 -m 12 -n 230
...
7. Assess grid search performances:
    ...
    When the grid search through parameters from step 1 has completed, use this script to calculate the performances (aucroc) of each classifier/parameter set combination
        ...
        ~john3784/Github/MachineLearning-Weka/machine_learning_pipeline_2-performance-balanced_arffs.py
...
8. get F measure
  		...
  		python ~john3784/Github/MachineLearning-Weka/performance_at_thresholds-pred2.py <pred file> <pos class name> <neg class name>
...
#applying models to known (manual) cv data and get scores:

1. first get run the manually cross validated files (all_cv.runcc) to get scores for your training set:

		python ~john3784/Github/parse_scripts/qsub_hpc-JL.py -f submit -c all_cv.runcc -u john3784 -n 230 -w 239 -m 12 -nc 20
...
2. apply models to cross validated samples 
		...
		python ~john3784/Github/MachineLearning-Weka/2_Machine_Learning/machine_learning_pipeline_3-apply_manual_cv_models-balanced_arffs.py
...
submit output file:
	...
	python ~john3784/Github/parse_scripts/qsub_hpc-JL.py -f submit -c all_apply_models.runcc -u john3784 -n 230 -w 239 -m 12 -nc 20

3. associate scores with cross-validated samples #this step requires scikit

		python ~john3784/Github/MachineLearning-Weka/2_Machine_Learning/machine_learning_pipeline_4-associate_gene_ids-balanced_arffs.py
...
5. get F-measure for balanced data to call threshold
	...
	first get random pulls from your negative dataset (will output 100 random files for each model)
		...
		python ~john3784/Github/MachineLearning-Weka/get_random_genes_scorefile.py <model .score file>
	...
	now get threshold from random sampling of data:
		...
		python ~john3784/Github/MachineLearning-Weka/performance_at_thresholds-pred2_randomfiles.py <start_dir with random files> <pos class name> <neg class name>
...
1. First you need the best_models.runcc file generated in step 7 (assessing grid search performance)
	...
	qsub this file:
		...
		python ~john3784/Github/parse_scripts/qsub_hpc.py  -f submit -u john3784 -c best_models.runcc -w 239 -m 12 -n 230
		
2. use the following script to generate another runcc file to apply best-performing models to an unlabled ARFF file:
	
		~john3784/Github/MachineLearning-Weka/machine_learning_pipeline_3b-apply_models_to_unlabeled-balanced_arff.py
...
4. Associate scores with instance identifiers - unlabeled instances
	...
	Once models have completed being applied to test sets, this script will assocaited teh machine learning scores with instance identifiers:
		...
		python ~john3784/Github/MachineLearning-Weka/2_Machine_Learning/machine_learning_pipeline_4b-associate_gene_ids-unlabeled-balanced_arffs.py  
...
5. You will need the F-measure cut off from manual-cv to call threshold on score file
...
#Extra/Archived instructions:

1. visualization scripts auc-roc
